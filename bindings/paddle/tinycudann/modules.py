...
def _paddle_precision(tcnn_precision):
...
class _module_function(paddle.autograd.PyLayer):
	@staticmethod
	def forward(ctx, native_tcnn_module, input, params, loss_scale):
		native_ctx, output = native_tcnn_module.fwd(input, params, input_requires_grad=True, params_requires_grad=True)
...
class _module_function_backward(paddle.autograd.PyLayer):
	@staticmethod
	def forward(ctx, ctx_fwd, doutput, input, params, output):
		...
		with paddle.no_grad():
			...
			input_grad, weight_grad = ctx_fwd.native_tcnn_module.bwd(ctx_fwd.native_ctx, input, params, output,
																	 scaled_grad, input_requires_grad=False,
	...
	def backward(ctx, dinput_grad, dweight_grad):
		...
		with paddle.no_grad():
			doutput_grad, weight_grad, input_grad = ctx.ctx_fwd.native_tcnn_module.bwd_bwd_input(
				...
				dinput_grad,
				doutput,
				dL_doutput_requires_grad=False,
				params_requires_grad=False,
...
class Module(paddle.nn.Layer):
	def __init__(self, seed=1337):
		...
		self.dtype = _paddle_precision(self.native_tcnn_module.param_precision())

		self.seed = seed
		initial_params = self.native_tcnn_module.initial_params(seed)
		self.params = paddle.create_parameter(initial_params.shape, initial_params.dtype,
											  default_initializer=Assign(initial_params))
	...
	def forward(self, x):
		...
		x_padded = x if batch_size == padded_batch_size else paddle.nn.functional.pad(x, [0, 0, 0, padded_batch_size - batch_size])
		output = _module_function.apply(
			self.native_tcnn_module,
			x_padded,
			self.params.astype(_paddle_precision(self.native_tcnn_module.param_precision())),
			self.loss_scale
		)
